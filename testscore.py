...
class StudyScoreApp:
    ...
    def create_widgets(self):
        ...
        # Removed Calculate Difficulty button
    ...
    def display_summary(self):
        ...
        tree.heading("subject",text="Subject"); tree.heading("average",text="Avg Score")
        for subj, scores in subject_scores.items():
            avg = round(sum(scores.values())/len(scores),2) if scores else 0
            tree.insert('',tk.END,values=(subj,avg))
        tree.pack(fill=tk.BOTH,expand=True,padx=10,pady=5)
        # Plot per SUBJECT selection
        cf=ttk.Frame(self.summary_tab); cf.pack(pady=5)
        ttk.Label(cf,text="Select Subject:").grid(row=0,column=0)
        sv=tk.StringVar(value=self.subjects[0] if self.subjects else "")
        cb=ttk.Combobox(cf,values=self.subjects,textvariable=sv,state='readonly'); cb.grid(row=0,column=1)
        cb.bind("<<ComboboxSelected>>",lambda e: self.plot_subject(sv.get(),subject_scores))
        self.fig,self.ax=plt.subplots(figsize=(6,3))
        canvas=FigureCanvasTkAgg(self.fig,master=self.summary_tab)
        canvas.get_tk_widget().pack(fill=tk.BOTH,expand=True,padx=10,pady=5)
        if self.subjects: self.plot_subject(self.subjects[0],subject_scores)
        self.canvas = canvas

    def display_difficulty(self):
        for w in self.diff_tab.winfo_children(): w.destroy()
        # read scores
        ...
        for r in res: tree.insert('',tk.END,values=r)
...
if __name__=="__main__":
    root=tk.Tk(); app=StudyScoreApp(root); root.mainloop()